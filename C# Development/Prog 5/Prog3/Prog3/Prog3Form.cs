...
namespace LibraryItems
{
    public partial class Prog3Form : Form
    {
        ...
        // Precondition:  None
        // Postcondition: Number of days is returned (loanperiod)
        //                
        private int FindLoanPeriod(string callnum)
        {
            for (int i = 0; i < _lib.GetItemsList().Count(); i++)
            {
                if (callnum == _lib.GetItemsList().ElementAt(i).CallNumber)
                    return _lib.GetItemsList().ElementAt(i).LoanPeriod;
            }
            return 0;
        }
            ...
            else
            {
                ...
                if (result == DialogResult.OK) // Only add if OK
                {
                    try
                    {
                        ...
                        _lib.CheckOut(checkoutForm.CallNum,checkoutForm.PatronID);
                    }
            ...
            else
            {
                ...
                if (result == DialogResult.OK) // Only add if OK
                {
                    try
                    {
                        ...
                        _lib.ReturnToShelf(returnForm.CallNum);
                    }
                    catch (ArgumentOutOfRangeException) // This should never happen
                    {
